# -*- coding: utf-8 -*-
"""
scripts/modal_evaluate_mtl_e4_held_out.py — Modal Cloud Runner for MTL E4 PCGrad Held-Out Evaluation
====================================================================================================
Dispatches the unified held-out evaluation of Phase 3 E4 PCGrad Multi-Task Learning Best Checkpoint
(`/mtl-checkpoints/mtl_e4_pcgrad/mtl_e4_best.pth`) on Modal profile 'hasinishrak2015'.

Evaluates:
  1. BCS on 7,549 ScienceDB test images (exact Run 4 matched identities; burst-group-disjoint)
  2. Behavior on 780 sequences (exact Run 5 matched retained population, CVB + Beef)
  3. Re-ID on SideViewCows2026 Protocol A (69 held-out cows; 36,811 Parlor gallery, 25,260 Barn queries, 607 Snapshots queries)

Usage:
    modal run --profile hasinishrak2015 scripts/modal_evaluate_mtl_e4_held_out.py::run_evaluation
"""
...
eval_image = (
    ...
    .add_local_file(
        str(REPO_ROOT / "scripts" / "evaluate_mtl_e4_held_out.py"),
        remote_path="/root/scripts/evaluate_mtl_e4_held_out.py",
    )
)

app = modal.App("mtl-e4-held-out-evaluation", image=eval_image)
...
def evaluate_held_out_remote() -> Dict[str, Any]:
    ...
    import torch
    from scripts.evaluate_mtl_e4_held_out import evaluate_run_e4_held_out

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    checkpoint_path = Path("/mtl-checkpoints/mtl_e4_pcgrad/mtl_e4_best.pth")
    ...
    output_dir = Path("/mtl-checkpoints/mtl_e4_evaluation")

    results = evaluate_run_e4_held_out(
...
def run_evaluation():
    print("=" * 75)
    print("  DISPATCHING PHASE 3 E4 PCGRAD HELD-OUT EVALUATION TO MODAL CLOUD")
    print("=" * 75)
    results = evaluate_held_out_remote.remote()

    local_out_dir = REPO_ROOT / "artifacts" / "mtl_e4_evaluation"
    local_out_dir.mkdir(parents=True, exist_ok=True)
    local_json = local_out_dir / "mtl_e4_test_evaluation_metrics.json"
