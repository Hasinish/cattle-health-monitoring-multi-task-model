# -*- coding: utf-8 -*-
"""
scripts/modal_evaluate_mtl_e1_held_out.py — Modal Cloud Runner for MTL E1 Held-Out Evaluation
=============================================================================================
Dispatches the unified held-out evaluation of Run 7 MTL E1 Best Checkpoint
(`/mtl-checkpoints/mtl_e1_hard_shared/mtl_e1_best.pth`) on Modal profile 'hasinishrak2015'.
...
Usage:
    modal run --profile hasinishrak2015 scripts/modal_evaluate_mtl_e1_held_out.py::run_evaluation
"""
...
eval_image = (
    ...
    .add_local_file(
        str(REPO_ROOT / "scripts" / "evaluate_mtl_e1_held_out.py"),
        remote_path="/root/scripts/evaluate_mtl_e1_held_out.py",
    )
)

app = modal.App("mtl-e1-held-out-evaluation", image=eval_image)
...
def evaluate_held_out_remote() -> Dict[str, Any]:
    ...
    import torch
    from scripts.evaluate_mtl_e1_held_out import evaluate_run7_held_out

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    checkpoint_path = Path("/mtl-checkpoints/mtl_e1_hard_shared/mtl_e1_best.pth")
    ...
    output_dir = Path("/mtl-checkpoints/mtl_e1_evaluation")

    results = evaluate_run7_held_out(
...
def run_evaluation():
    print("=" * 75)
    print("  DISPATCHING RUN 7 E1 HELD-OUT EVALUATION TO MODAL CLOUD")
    print("=" * 75)
    results = evaluate_held_out_remote.remote()

    local_out_dir = REPO_ROOT / "artifacts" / "mtl_e1_evaluation"
    local_out_dir.mkdir(parents=True, exist_ok=True)
    local_json = local_out_dir / "mtl_e1_test_evaluation_metrics.json"
