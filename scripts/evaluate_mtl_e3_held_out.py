# -*- coding: utf-8 -*-
"""
scripts/evaluate_mtl_e3_held_out.py — Official Held-Out Evaluation of Run 8 MTL E3 Best Checkpoint
===================================================================================================
Evaluates the official frozen Run 8 E3 modular MTL best checkpoint:
  `/mtl-checkpoints/mtl_e3_modular/mtl_e3_best.pth`
...
Zero test leakage:
  - Exact frozen Epoch-2 checkpoint selected by validation objective (val_e3_objective = 0.38888).
  - No retraining, no tuning, no threshold adjustments.
  - Matches Run 4, Run 5, Run 6, and Run 7 evaluation protocols exactly.
"""
...
def evaluate_retrieval_chunked(
    ...
    Matches Run 6 and SideViewCows2026 Protocol A exactly.
    """
...
def evaluate_run8_held_out(
    ...
    print("\n" + "=" * 80)
    print("  OFFICIAL PHASE 3 RUN 8 (E3 MODULAR MTL) HELD-OUT EVALUATION")
    print(f"  Checkpoint: {checkpoint_path}")
    print(f"  Device:     {device} ({torch.cuda.get_device_name(0) if device.type == 'cuda' else 'CPU'})")
    print("=" * 80)

    # 1. Load frozen model checkpoint
    print(f"\n[*] Loading frozen E3 Best Checkpoint from {checkpoint_path}...")
    assert checkpoint_path.exists(), f"Missing checkpoint: {checkpoint_path}"
    ckpt = torch.load(checkpoint_path, map_location=device, weights_only=False)

    model = MTLE3ModularModel(pretrained=False).to(device)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    val_obj = ckpt.get("best_val_objective", ckpt.get("val_e3_objective", 0.0))
    epoch_val = ckpt.get("epoch", "N/A")
    print(f"  [+] Loaded Epoch {epoch_val} checkpoint (ValObj = {val_obj:.5f})")

    # ==========================================================================
    # TASK 1: BCS ON EXACT RUN 4 MATCHED TEST IDENTITIES (N=7,549)
    # ==========================================================================
    print("\n" + "-" * 80)
    print("  [TASK 1/3] EVALUATING BCS ON MATCHED TEST IDENTITIES (N=7,549)")
    print("-" * 80)
    ...
    bcs_raw_labels = bcs_payload["raw_labels"]  # [7549] real physical BCS
    ...
    # ==========================================================================
    # TASK 2: BEHAVIOR ON EXACT RUN 5 RETAINED POPULATION (N=780, T=8)
    # ==========================================================================
    print("\n" + "-" * 80)
    ...
    # Preload all 780 sequences into RAM
    t0_beh = time.time()
    ...
    if cached_beh_pt.exists():
        ...
        cached_payload = torch.load(cached_beh_pt, map_location="cpu")
    ...
    # ==========================================================================
    # 3. FAIR MATCHED DELTAS & COMPARISONS AGAINST RUNS 4, 5, 6, AND RUN 7 E1
    # ==========================================================================
    ...
    # Run 7 E1 Hard-Shared MTL Certified Held-Out Baseline:
    run7_e1_baseline = {
        "bcs": {
            "real_mae": 0.1788,
            "acc_0": 0.4110,
        ...
        "behavior": {
            ...
            "cvb_metrics": {"overall_accuracy": 0.7678, "balanced_accuracy": 0.5402, "macro_f1": 0.5402},
            "beef_metrics": {"overall_accuracy": 0.9469, "balanced_accuracy": 0.9236, "macro_f1": 0.9236},
        },
        "reid": {
    ...
    full_evaluation_payload = {
        "experiment": "Phase 3 Run 8 E3 Modular MTL Official Held-Out Evaluation",
        "evaluated_checkpoint": str(checkpoint_path),
        "checkpoint_selection_criterion": "Predefined Minimum Validation E3 Objective (Epoch 2, val_e3_objective = 0.38888)",
        ...
        "reid": {
            "run8_e3_protocol_a": reid_results,
    ...
    out_json = output_dir / "mtl_e3_test_evaluation_metrics.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Official Phase 3 Run 8 E3 Held-Out Evaluation")
    parser.add_argument("--checkpoint-path", type=Path, default=Path("/mtl-checkpoints/mtl_e3_modular/mtl_e3_best.pth"))
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/mtl-checkpoints/mtl_e3_evaluation"))
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    evaluate_run8_held_out(
