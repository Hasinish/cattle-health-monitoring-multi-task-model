# -*- coding: utf-8 -*-
"""Cloud-to-Cloud Transfer of Certified Viewpoint Checkpoint.
...
To:
  Destination: dryousufmozumder, volume 'reid-checkpoints'
...
def transfer_checkpoint() -> Dict[str, Any]:
    print("=" * 70)
    print("  🚀 STEP A: TRANSFER CERTIFIED VIEWPOINT CHECKPOINT")
    print("  Source     : tigerwood693 (viewpoint-checkpoints)")
    print("  Destination: dryousufmozumder (reid-checkpoints)")
    print("=" * 70)

    src_creds = get_profile_creds("tigerwood693")
    dst_creds = get_profile_creds("dryousufmozumder")
    ...
    dst_vol = modal.Volume.from_name("reid-checkpoints", client=dst_client)
    ...
    print(f"\n[3/4] Uploading to dryousufmozumder -> {dst_rel_path}...")
    t_up = time.time()
    ...
    print(f"\n[4/4] Verifying destination file on dryousufmozumder...")
    ...
    res = {
        ...
        "destination_profile": "dryousufmozumder",
        "destination_volume": "reid-checkpoints",
...
if __name__ == "__main__":
    result = transfer_checkpoint()
    print("\n" + "=" * 70)
    print("  🎉 STEP A COMPLETE: CHECKPOINT SUCCESSFULLY TRANSFERRED")
