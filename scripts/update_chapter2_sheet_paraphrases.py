...
from scripts.integrate_chapter2_paraphrases import prefixes, paraphrased_list

# Plain text versions (no LaTeX commands) for Google Sheet
plain_text_paras = [
    ...
    "BCS Automation is highly dependent on the cow’s anatomy. Edmonson et al. proposed an index of scoring while Ferguson et al. investigated the correlation between anatomical traits and certain BCS values. From a point of view of computer vision, considering the whole body mass of the cow is not sufficient. In order for the model to work, it should detect the amount of tissue covering the body, body shape, and specific body parts. This is why the camera position and selected body area matter.",
    "Another approach uses depth images for better representation of the body shape of a cow. In particular, Rodriguez Alvarez et al. employ convolutional neural networks along with depth images and validate the prediction of BCS by means of various error thresholds. This approach relies on the combination of body geometry and features rather than the handcrafted features alone. The paper also proves that exact accuracy and tolerance-based accuracy have to be evaluated separately because of the fact that BCS scores are ordinal.",
    "Another methodology makes use of ordinary RGB images to detect visual cues associated with body condition. In the case of Liu et al., most of the feature extraction takes place in the tail area and is done using the EfficientNet-B0 architecture with channel and spatial attention. Their work involves the selection of a specific body part and an efficient network architecture, while distillation assists in making the model size smaller. The fact that BCS prediction can be tailored through a combination of the specific body part and architecture has been illustrated.",
    "Camera view influences the information that can be obtained by the model as well. Yao et al. select side-view pictures of the cattle using a ConvNeXt-based regression model and cow-level cross-validation. One method is focused on the tail of the animal; therefore, it observes just a small portion of the body, whereas another method selects side-view pictures and considers a bigger part of the cow’s body. Both methods are different not only in their models but also in the parts of the cow they observe.",
    "The research done on cattle measurement also reveals that there is a need for distinction between image size and true physical measurements. The authors Guzhva et al. came up with a system called PickAMoo, which uses smartphone images, LiDAR scaling, and Mask R-CNN segmentation in order to estimate weight. Even though the purpose of estimating the weight is different from estimating BCS, this method is relevant since the estimation is done when the animal has been separated from the image, and pixels are related to physical measurements.",
    "The common factor among all these papers can be seen in three aspects that make good components for BCS prediction. First is the use of images that have proper features of the body and its orientation. Second is the use of an appropriate technique to extract such information, for example, cropping, body contour, depth map, or attention. Third is selecting an appropriate prediction technique that is suited to ordered BCS. CORAL can handle ordering of output classes, but it will not perform well if the input class does not have appropriate condition features. Thus, body-shape information will be considered as part of the representation, and ordinal prediction as the model choice.",
    ...
    "There are several modern object detectors that can help to locate this region. RT-DETR developed by Zhao et al. uses an end-to-end transformer detector optimized for speed-accuracy trade-off. Instance segmentation approaches are richer than bounding box. Mask R-CNN extends the detection network with the mask branch and thus makes it predict not only the position but also object specific mask. To be more precise, the bounding box says where the cow is, while the mask defines its pixels.",
    "Promptable segmentation decouples the process of selecting the target from creating its mask. Promptable segmentation model, Segment Anything by Kirillov et al., is capable of generating masks for prompts such as points and boxes. Promptable segmentation in SAM 2, being memory-based, is applicable to both images and video. This implies that in a cattle pipeline, for instance, we can first detect the cow, and then prompt the segmentation model to generate the mask, without the need to train a new cattle-specific segmentation model from scratch.  The final mask will depend upon how the subject was chosen and prompted.",
...
def main():
    service = get_service()
    sheet_title = "Chapter 2: Literature Review"
    ...
    # Read rows B54:B260
    res = service.values().get(spreadsheetId=SPREADSHEET_ID, range=f"'{sheet_title}'!B54:B260").execute()
    vals = res.get('values', [])
    ...
    for i, r in enumerate(vals):
        ...
        if len(r) > 0:
            ...
            for idx, pref in enumerate(prefixes):
                if pref in cell_text:
                    matched_rows[idx] = row_num
                    break

    print(f"Matched {len(matched_rows)}/31 rows in {sheet_title}")

    # Prepare batch update for values in Col C
    data = []
    for idx in range(31):
        row_num = matched_rows[idx]
        para_text = plain_text_paras[idx]
        data.append({
            "range": f"'{sheet_title}'!C{row_num}",
            "values": [[para_text]]
        })
    ...
    print("Updated Col C values for all 31 rows!")

    # Un-highlight row 54 (which was previously truncated)
    # Reset background color of row 54 Col C (0-indexed row 53, col 2)
    format_req = {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
    ...
    service.batchUpdate(spreadsheetId=SPREADSHEET_ID, body={"requests": [format_req]}).execute()
    print("Reset Row 54 Col C background to white!")
