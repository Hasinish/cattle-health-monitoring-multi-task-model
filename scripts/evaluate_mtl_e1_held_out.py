# -*- coding: utf-8 -*-
"""
scripts/evaluate_mtl_e1_held_out.py — Official Held-Out Evaluation of Run 7 MTL E1 Best Checkpoint
===================================================================================================
Evaluates the official frozen Run 7 E1 hard-shared MTL best checkpoint:
  `/mtl-checkpoints/mtl_e1_hard_shared/mtl_e1_best.pth`
...
Zero test leakage:
  - Exact frozen Epoch-3 checkpoint selected by validation objective (val_e1_objective = 0.40036).
  - No retraining, no tuning, no threshold adjustments.
  - Matches Run 4, Run 5, and Run 6 evaluation protocols exactly.
"""
...
# ------------------------------------------------------------------------------
# 1. Re-ID Protocol A Dataset & Retrieval Evaluation
# ------------------------------------------------------------------------------
...
# SideViewGTMaskReIDDataset imported directly from scripts.train_sideview_reid_perception
...
def evaluate_run7_held_out(
    ...
    print("\n" + "=" * 80)
    print("  OFFICIAL PHASE 3 RUN 7 (E1 HARD-SHARED MTL) HELD-OUT EVALUATION")
    print(f"  Checkpoint: {checkpoint_path}")
    print(f"  Device:     {device} ({torch.cuda.get_device_name(0) if device.type == 'cuda' else 'CPU'})")
    print("=" * 80)

    # 1. Load frozen model checkpoint
    print(f"\n[*] Loading frozen E1 Best Checkpoint from {checkpoint_path}...")
    assert checkpoint_path.exists(), f"Missing checkpoint: {checkpoint_path}"
    ckpt = torch.load(checkpoint_path, map_location=device, weights_only=False)
    
    model = MTLE1HardSharedModel(pretrained=False).to(device)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    val_obj = ckpt.get("best_val_objective", ckpt.get("val_e1_objective", 0.0))
    ...
    # ==========================================================================
    # 3. FAIR MATCHED DELTAS & COMPARISONS AGAINST RUNS 4, 5, AND 6
    # ==========================================================================
    # Baselines:
    run4_baseline = {
        "population": 7549,
    ...
    full_evaluation_payload = {
        "experiment": "Phase 3 Run 7 E1 Hard-Shared MTL Official Held-Out Evaluation",
        "evaluated_checkpoint": str(checkpoint_path),
        "checkpoint_selection_criterion": "Predefined Minimum Validation E1 Objective (Epoch 3, val_e1_objective = 0.40036)",
        ...
        "bcs": {
            "run7_e1_matched": bcs_results,
            "run4_baseline_matched": run4_baseline,
            "deltas": bcs_deltas,
            "verdict": "Degradation" if bcs_deltas["real_mae_delta"] > 0 else "Improvement",
        },
        "behavior": {
            "run7_e1_matched": beh_results,
            "run5_baseline_matched": run5_baseline,
            "deltas": beh_deltas,
            "verdict": "Improvement" if beh_deltas["balanced_accuracy_delta_pct"] > 0 else "Degradation",
        },
        "reid": {
            "run7_e1_protocol_a": reid_results,
            "run6_baseline_protocol_a": run6_baseline,
            "deltas": reid_deltas,
            "verdict": "Improvement" if (reid_deltas["query_barn"]["rank_1_delta"] >= 0 and reid_deltas["query_snapshots"]["rank_1_delta"] >= 0) else ("Mixed" if (reid_deltas["query_barn"]["rank_1_delta"] >= 0 or reid_deltas["query_snapshots"]["rank_1_delta"] >= 0) else "Degradation"),
        },
    }

    output_dir.mkdir(parents=True, exist_ok=True)
    out_json = output_dir / "mtl_e1_test_evaluation_metrics.json"
