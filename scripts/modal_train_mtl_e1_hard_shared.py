# -*- coding: utf-8 -*-
"""
scripts/modal_train_mtl_e1_hard_shared.py — Modal Cloud Wrapper for Phase 3 Run 7: E1 Hard-Shared MTL Control
==============================================================================================================
Target Profile   : hasinishrak2015
Mounted Volumes  :
  - mtl-data        -> /mtl-data        (BCS monolithic tensors + Behavior 4,271 sequences + staging_manifest.json)
  - mtl-checkpoints -> /mtl-checkpoints (Run 7 checkpoints and metrics)
  - sideview-data   -> /sideview        (SideViewCows2026 parlor dataset under Zero-Copy Policy)

Hardware Targets:
  - verify_readiness: CPU only (cpu=2.0, memory=8192 MB, NO GPU)
  - smoke_test      : NVIDIA T4 (Low-cost cloud GPU smoke verification)
  - main / full run : NVIDIA L40S (48GB Ada Lovelace tier, cpu=8.0, memory=32768 MB)

Usage:
  1. Zero-GPU Readiness Verification:
     modal run --profile hasinishrak2015 scripts/modal_train_mtl_e1_hard_shared.py::verify_readiness

  2. Cloud GPU Smoke Test (2 Epochs, Cheap T4 GPU, Train/Val Only, Held-Out Untouched):
     modal run --profile hasinishrak2015 scripts/modal_train_mtl_e1_hard_shared.py::smoke_test
...
app = modal.App("mtl-e1-hard-shared-run7", image=mtl_image)
...
def verify_readiness_remote() -> Dict[str, Any]:
    ...
    parameter counts, and container imports before launching GPU smoke test.
    ...
    import torch
    import pandas as pd
    from scripts.train_mtl_e1_hard_shared import MTLE1HardSharedModel

    print("=" * 78)
    print("  PHASE 3 RUN 7: READINESS & PRE-FLIGHT VERIFICATION (PROFILE: hasinishrak2015)")
    print("  Experiment: E1 Hard-Shared Multi-Task Learning Control")
    print("=" * 78)
    ...
    # 1. Checkpoint Volume Writeability
    checkpoints_dir = Path("/mtl-checkpoints")
    assert checkpoints_dir.exists(), "Missing /mtl-checkpoints mount"
    probe_p = checkpoints_dir / ".probe_run7.tmp"
    with open(probe_p, "w") as f:
        f.write("OK_RUN7")
    assert probe_p.exists() and probe_p.read_text() == "OK_RUN7"
    probe_p.unlink()
    mtl_checkpoints_vol.commit()
    report["checks"]["checkpoints_volume_writable"] = True
    print("[*] mtl-checkpoints volume: Writable and verified ✅")
    ...
    assert not (bcs_dir / "test_bcs_224.pt").exists(), "LEAKAGE: test_bcs_224.pt present!"
    ...
    # 6. Model Architecture & Programmatic Hard Sharing
    model = MTLE1HardSharedModel(pretrained=False)
    model.assert_hard_sharing()
    counts = model.count_parameters()
    report["checks"]["parameter_counts"] = counts
    print(f"[*] MTLE1HardSharedModel: {counts['total_trainable_parameters']:,} parameters (Backbone: {counts['shared_backbone_parameters']:,}) ✅")
    print(f"[*] Hard sharing programmatically proven: EXACTLY ONE ResNet-18 visual trunk ✅")

    print("\n" + "=" * 78)
    print("  ALL READINESS CHECKS PASSED — WORKSPACE IS 100% READY FOR GPU SMOKE TEST")
    print("=" * 78)
...
def smoke_test_remote(active_git_sha: str = "") -> Dict[str, Any]:
    ...
    Verifies:
      - BCS, Behavior, Re-ID forward & backward
    ...
    """
    from scripts.train_mtl_e1_hard_shared import train_mtl_e1_pipeline

    print("=" * 78)
    print("  PHASE 3 RUN 7: REMOTE GPU SMOKE TEST ON NVIDIA T4 (PROFILE: hasinishrak2015)")
    print("=" * 78)

    smoke_output_dir = Path("/mtl-checkpoints/mtl_e1_smoke")
    results = train_mtl_e1_pipeline(
...
def train_mtl_e1_full_remote(
    ...
    """
    Executes the full 30-epoch Run 7 E1 Hard-Shared MTL training run on NVIDIA L40S.
    """
    from scripts.train_mtl_e1_hard_shared import train_mtl_e1_pipeline

    print("=" * 78)
    print("  PHASE 3 RUN 7: FULL 30-EPOCH E1 HARD-SHARED MTL ON NVIDIA L40S")
    print(f"  Profile          : hasinishrak2015")
    print(f"  Target Epochs    : {epochs}")
    print(f"  Git Commit SHA   : {active_git_sha}")
    print("=" * 78)

    output_dir = Path("/mtl-checkpoints/mtl_e1_hard_shared")
    results = train_mtl_e1_pipeline(
        bcs_data_dir=Path("/mtl-data/bcs"),
        behavior_data_dir=Path("/mtl-data/behavior"),
        reid_data_dir=Path("/sideview/sideviewcows2026"),
    ...
    mtl_checkpoints_vol.commit()
    print("[*] Committed full Run 7 checkpoints and metrics to mtl-checkpoints volume ✅")
    return results
...
def verify_readiness():
    """modal run --profile hasinishrak2015 scripts/modal_train_mtl_e1_hard_shared.py::verify_readiness"""
    print("\n[Local] Dispatching Readiness Verification to Modal (Zero GPU)...")
    res = verify_readiness_remote.remote()
    print("\n[Local] Readiness Report Summary:")
...
def main(epochs: int = 30, batch_size_bcs: int = 64, batch_size_beh: int = 8, batch_size_reid: int = 32):
    """modal run --detach --profile hasinishrak2015 scripts/modal_train_mtl_e1_hard_shared.py::main --epochs 30"""
    sha = get_git_commit_sha()
    print(f"\n[Local] Dispatching Full 30-Epoch Run 7 Training to Modal (NVIDIA L40S) [Git SHA: {sha}]...")
    res = train_mtl_e1_full_remote.remote(
        epochs=epochs,
        batch_size_bcs=batch_size_bcs,
        batch_size_beh=batch_size_beh,
        batch_size_reid=batch_size_reid,
