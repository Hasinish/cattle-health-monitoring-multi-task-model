...
The framework relies only on public datasets, grouped by task. For Body Condition Scoring (BCS), the datasets are the ScienceDB BCS Dataset, which has about 53,000 RGB images labeled on a 5-point scale, and the Dryad RGB+Depth Dataset with paired RGB and depth images for BCS. The MmCows Dataset provides RGB video with 7 behavior classes that includes standing, walking, feeding up and down, drinking, licking, and lying for behavior recognition, and the CBVD-5 Dataset provides RGB video with 5 behavior categories for cross-dataset evaluation. The CattleLameness Dataset contains RGB video with lameness severity annotations for lameness detection. OpenCows2020 includes RGB images labeled by individual identity for individual identification.


Data preprocessing includes resizing images to $224 \times 224$, normalization using ImageNet statistics, data augmentation through horizontal flip and rotation ($\pm 15^{\circ}$), along with cow-wise splitting to prevent data leakage by ensuring individuals in the test set never appear in training.
