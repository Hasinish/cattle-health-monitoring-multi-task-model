...
\toprule
\textbf{Member} & \textbf{Backbone} & \textbf{Dryad MAE} & \textbf{Dryad Exact} & \textbf{Dryad $\pm 1$} & \textbf{SciDB MAE} & \textbf{SciDB Exact} & \textbf{SciDB $\pm 1$} \\
\midrule
...
EfficientNetB0 achieves the best validation and test Macro F1-score (per-class breakdown is provided in Table~\ref{tab:behavior_per_class}). DenseNet121 is the second-best behavior model.
...
\toprule
\textbf{Training Setup} & \textbf{BCS MAE} & \textbf{Behavior F1} & \textbf{Lameness Acc} & \textbf{Lameness AUC} & \textbf{ID Accuracy} \\
\midrule
...
\midrule
BCS (MAE) & Liang et al. (2025) & Private dataset & 93.77\% accuracy & 0.5566 MAE (ScienceDB) \\
Behavior (F1) & Dottorini et al. (2025) & MmCows & 91.8\% accuracy & 0.7445 Macro F1 \\
