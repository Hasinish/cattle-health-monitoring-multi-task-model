...
- `evaluate_mtl_e4_held_out.py`: Phase 3 E4 PCGrad official held-out evaluation engine, evaluating the frozen Epoch-3 checkpoint (`mtl_e4_best.pth`, `val_e4_objective = 0.40098`, 11,926,706 trainable params) across BCS (7,549 ScienceDB images), Behavior (780 retained sequences), and Re-ID Protocol A (69 held-out cows; 36,811 gallery, 25,260 barn, 607 snapshots), computing 3-way deltas vs single-task baselines, Run 7 E1, and Run 8 E3.
- `modal_evaluate_mtl_e4_held_out.py`: Modal cloud evaluation runner for E4 PCGrad held-out evaluation on profile `hasinishrak2015` (NVIDIA L40S), mounting `mtl-checkpoints`, `mtl-data`, and `sideview-data`.
- `stage_mtl_workspace.py`: Master local controller for maximum-speed, resumable, low-disk MTL data staging on Modal profile `hasinishrak2015`. Manages BCS and Behavior chunk relay (<= 1 GB local buffer, SHA-256 verification, immediate local chunk deletion), orchestrates 16-worker direct cloud Re-ID download inside `hasinishrak2015`, provides live `CleanProgressBar` with %/MB/s/ETA, and verifies zero test leakage.
- `modal_export_bcs.py`: Dedicated profile-isolated BCS export engine running strictly on `tigerwood697` mounting `sciencedb-perception-cache`. Chunks monolithic binary tensors (`train_bcs_224.pt`, `val_bcs_224.pt`) into 1024MB parts with SHA-256 checksums and cleans up temporary volume exports.
- `modal_export_behavior.py`: Dedicated profile-isolated Behavior export engine running strictly on `tigerwood693` mounting `behavior-perception-cache`. Packages 4,271 retained sequence directories into an uncompressed sequential tar archive, chunks into 1024MB parts with SHA-256 checksums, and cleans up temporary volume exports.
- `modal_export_mtl_sources.py`: Backward-compatible redirect pointer documenting Modal profile-volume scoping separation between BCS (`tigerwood697`) and Behavior (`tigerwood693`).
- `modal_stage_mtl_target.py`: Target-side reassembly and direct hydration engine on `hasinishrak2015` attaching `mtl-data` and `mtl-checkpoints`. Reassembles BCS tensors, extracts Behavior archive, executes direct 16-worker HTTP Range download of `parlor.zip` from Zenodo to ephemeral `/tmp/`, selectively extracts 15,436 Train/Val pairs across 41 cows, purges temporary archive, and runs comprehensive audit producing `/mtl-data/staging_manifest.json`.
- `modal_hydrate_dryad.py`: Modal hydration and verification pipeline for Dryad Cattle BCS dataset on volume `dryad-bcs-data` (5,940 DGE TIFFs across classes 2–7, 54 biological cows, 854 MB).
- `modal_hydrate_ruchay.py`: Modal hydration and verification pipeline for Ruchay 2026 BCS dataset on volume `ruchay-data` (77.74 GB raw archives across 5 sessions + `Dataset.xlsx`, 1,025 cows, 25,700 RGB-D pairs).
- `modal_hydrate_cbvd5.py`: Modal hydration and verification pipeline for CBVD-5 behavior dataset on volume `cbvd5-data` (887 MP4 videos, 206,100 mini frames, 14.8 GB).
- `modal_hydrate_beca.py`: Modal hydration and verification pipeline for BECA dataset on volume `beca-data` (18.91 GB archive, BECA-D 16,889 images, BECA-L 12,172 images, 5,764 cows).
- `modal_hydrate_opencows.py`: Modal hydration and verification pipeline for OpenCows2020 dataset on volume `opencows-data` (4,736 images across 46 cows, 3.2 GB).
- `train_sideview_reid_perception.py`: Phase 3 Run 6 SideViewCows2026 GT/oracle-mask Re-ID trainer. Derives a 5%-margin target crop from each paired GT mask, applies synchronized RGB/mask geometry, builds 4-channel `[R,G,B,binary mask]` inputs, expands only ResNet-18 conv1 (+3,136 params), enforces Protocol A isolation in smoke mode, and verifies shapes, gradients, parameter counts, and bit-identical checkpoint reload.
- `build_behavior_perception_cache.py`: Real Behavior perception cache generator for Phase 3 Run 5. Implements exact CVB ground-truth target tracklet bounding-box lookups (`CVBExactAnnotationCache`) with full-frame SAM 2.1 Small segmentation; implements Kaggle Beef RT-DETR-L largest box + positive center point prompting (A5) with center point (112, 112) fallback. Supports deterministic benchmark candidate selection (`get_benchmark_sequence_subset`), interruption-safe resume, corrupted folder purge, periodic volume commits, and progressive manifest persistence (`save_cache_manifest_and_summary`).
- `train_cvb_beef_behavior_tcn.py`: Phase 3 Run 5 Behavior Temporal Core, Perception Integration, and Matched Evaluation engine. Supports dual modes: `input_mode="rgb"` (historical 3-channel baseline, 11,900,485 params) and `input_mode="rgb_mask"` (real Run 5 4-channel perception model, 11,903,621 params). Couples ImageNet-pretrained ResNet-18 (512-D per frame) with lightweight 1D TCN (2 Conv1d blocks, GELU, BatchNorm1d, Dropout=0.2, AdaptiveAvgPool1d, Linear head). Implements deterministic T=8 temporal sampling, authentic CVB target tracklet bbox preservation, and visual perception contact sheet generation. Includes `MatchedBehaviorRGBDataset` and `evaluate_matched_run2_vs_run5` to evaluate Run 5 Perception+TCN against the existing Run 2 RGB baseline checkpoint on the identical retained test sample IDs, outputting 3-way metrics and structured markdown reports.
- `modal_train_cvb_beef_behavior_tcn.py`: Complete Modal cloud execution suite for Phase 3 Run 5 on profile `tigerwood693`, mounting `cvb-data`, `beef-behavior-data`, `behavior-perception-cache`, and `behavior-checkpoints`. Implements entrypoints for: 1) L4/L40S ~5-min cache speed benchmarks (`benchmark_cache_l4`, `benchmark_cache_l40s`), 2) full Train+Val production caching on L4/L40S (`build_production_cache_l4`, `build_production_cache_l40s`), 3) full 30-epoch training (`train_full_run5`), 4) strict test gate & matched Run 2 evaluation (`evaluate_test_run5`), 5) smoke cache audit verification (`audit_smoke_cache`), and 6) zero-GPU cache/checkpoint status inspection (`inspect_cache_status`).
...
- `mtl_e4_evaluation/`: Phase 3 E4 PCGrad MTL official held-out evaluation artifacts on Modal profile `hasinishrak2015` (NVIDIA L40S). Contains `mtl_e4_test_evaluation_metrics.json` (evaluated across 7,549 ScienceDB BCS test images, 780 Behavior retained sequences, and 69 SideView Protocol A held-out cows; full metrics, confusion matrices, and 3-way deltas vs single-task baselines, Run 7 E1, and Run 8 E3).
- `behavior_viewpoint_audit/`: Phase 3 CVB + Kaggle Beef Behavior viewpoint feasibility audit artifacts on Modal profile `hasinishrak2015` (Tesla T4). Contains `behavior_viewpoint_frame_results.csv` (432 evaluated frames across 54 sequences), `behavior_viewpoint_sequence_summary.csv` (54 sequences), `behavior_viewpoint_transfer_sanity_metrics.json` (transfer sanity metrics, breakdowns by source, class, and source x class cells, TVD = 0.7646 shortcut risk), and `behavior_viewpoint_all_classes_contact_sheet.jpg` (visual contact sheet showing all 9 cells across T=8 chronological frames with predicted viewpoints).
- `behavior_viewpoint_smoke/`: Phase 3 Run 5 Behavior + Viewpoint controlled smoke test artifacts on profile `hasinishrak2015` (Tesla T4). Contains `behavior_viewpoint_smoke_metrics.json` (architecture parameter counts [11,920,405 trainable, 11,178,051 frozen, 23,098,456 total], 2-epoch smoke loss trajectory 1.8819 -> 0.6371, zero viewpoint gradients, and bit-identical checkpoint reload `max_logit_diff == 0.00000000`).
- `behavior_pose_audit/`: Phase 3 CVB + Kaggle Beef Behavior SuperAnimal-Quadruped pose feasibility audit artifacts on Modal profile `hasinishrak2015` (Tesla T4). Contains `behavior_pose_frame_results.csv` (432 evaluated frames across 54 sequences), `behavior_pose_sequence_summary.csv` (54 sequences), `behavior_pose_aggregate_metrics.json` (aggregate rates, breakdowns by source, class, cell, and keypoint rankings), `superanimal_39_keypoints_schema.json` (official 39-keypoint schema reference and skeleton connections), and `behavior_pose_all_classes_contact_sheet.jpg` (2064x2408 9-cell visual contact sheet showing all 9 cells across all T=8 chronological frames with predicted mask contours and keypoints).
- `mtl_e1_training/`: Phase 3 Run 7 E1 Hard-Shared Multi-Task Learning full 30-epoch training artifacts on Modal profile `hasinishrak2015` (NVIDIA L40S). Contains `mtl_e1_metrics.json` (complete 30-epoch training history, best epoch 3 metrics [ValObj 0.40036], peak Behavior F1 0.8012 at Epoch 8, peak Re-ID Top-1 96.65% at Epoch 25, BCS Real MAE 0.1950, and bit-identical checkpoint reload verification `max_logit_diff == 0.00000000`). Large model checkpoints (`mtl_e1_best.pth`, `mtl_e1_latest.pth`) are committed to `/mtl-checkpoints/mtl_e1_hard_shared/` on persistent volume `mtl-checkpoints`.
...
- `reid_perception_smoke/`: Run 6 local GTX 1050 Ti smoke outputs. `reid_perception_smoke_metrics.json` records the 128-pair integrity audit, protocol isolation, tensor/embedding/logit shapes, parameter delta, loss history, runtime, and bit-identical checkpoint reload. Local `.pth` checkpoints and the artifact-side JPG are intentionally ignored; the contact sheet is preserved under `docs/audits/assets/reid_perception_smoke/`.
- `behavior_perception_equivalence/`: Phase 3 Run 5 single-GPU fast perception caching equivalence audit artifacts on Modal profile `tigerwood693` (NVIDIA L40S). Contains `fast_path_equivalence_report.json` and `fast_path_equivalence_report.md` proving 100.00% exact binary mask equality (304/304 masks), min IoU 1.000000, mean IoU 1.000000, max RGB diff 0, and 100% agreement on dataset partitioning (38 retained, 2 excluded) between serial reference and fast batched/monotonic path.
- `behavior_perception_smoke/`: Phase 3 Run 5 Behavior perception smoke test artifacts on profile `tigerwood693` (NVIDIA T4). Contains `perception_manifest.csv` and `perception_summary.json` documenting the re-certified 304 authentic masks (192 CVB GT, 93 Kaggle Beef A5, 19 Kaggle Beef fallback) across 38 retained sequences with zero placeholder provenance records.
...
- `train_mtl_e4_pcgrad.py`: Standalone PyTorch engine for Phase 3 E4 PCGrad Multi-Task Learning (`MTLE4PCGradModel`), featuring exact E1 hard-shared 4-channel ResNet-18 spatial backbone (`11,179,648` params), task heads (`BCSOrdinalHead`, `BehaviorTemporalTCN`, `ReIDHead`; `747,058` params total), deterministic `PCGradProjector` operating strictly on shared backbone gradients (task heads unprojected with zero cross-talk), deterministic task-balanced super-step schedule (537 steps/epoch, fixed 1/1/1 weights), and comprehensive per-epoch gradient diagnostics (norms, cosines, conflict fractions). Total trainable parameters: `11,926,706` (asserted match to E1).
- `modal_train_mtl_e4_pcgrad.py`: Modal cloud dispatch wrapper for E4 PCGrad on profile `hasinishrak2015`, mounting `/mtl-data`, `/mtl-checkpoints` (dedicated `/mtl-checkpoints/mtl_e4_pcgrad/` and smoke `/mtl-checkpoints/mtl_e4_pcgrad_smoke/`), and `/sideview`. Implements CPU `verify_readiness`, Tesla T4 `smoke_test`, and L40S `main` full 30-epoch training entrypoint.
- `sync_mtl_e4_artifacts.py`: Automated artifact download utility fetching `mtl_e4_metrics.json` directly from Modal volume `mtl-checkpoints` on `hasinishrak2015` into `artifacts/mtl_e4_training/`.
- `train_mtl_e3_modular.py`: Standalone PyTorch engine for Phase 3 Run 8 E3 Modular Multi-Task Learning (`MTLE3ModularModel`), featuring shared 4-channel ResNet-18 spatial backbone (`11,179,648` params, 90.72%), three lightweight task-private residual bottleneck adapters (`131,968` params each = `395,904` total, 3.21%; zero-weight identity initialization), task heads (`BCSOrdinalHead`, `BehaviorTemporalTCN`, `ReIDHead`; `747,058` params total, 6.06%), deterministic task-balanced super-step schedule (537 steps/epoch, fixed 1/1/1 weights), and evaluation suite. Total trainable parameters: `12,322,610` (+3.32% over E1).
- `modal_train_mtl_e3_modular.py`: Modal cloud dispatch wrapper for Run 8 E3 on profile `hasinishrak2015`, mounting `/mtl-data`, `/mtl-checkpoints` (dedicated `/mtl-checkpoints/mtl_e3_modular/`), and `/sideview`. Implements CPU `verify_readiness`, Tesla T4 `smoke_test`, and L40S `main` full 30-epoch training entrypoint.
...
- `train_sideview_reid_viewpoint.py`: SideViewCows2026 Re-ID + Viewpoint ablation training script featuring 4-channel ResNet-18 visual trunk (`11,179,648` params), frozen viewpoint ResNet-18 (`11,178,051` params), Viewpoint MLP branch (`400` params, 3 -> 16), unit-L2 528-D fused embedding, and `Linear(528, 41)` classifier (`21,689` params). Total trainable parameters: `11,201,737` (+1,056 params / +0.0094% vs Run 6).
- `modal_train_sideview_reid_viewpoint.py`: Modal cloud wrapper for Re-ID + Viewpoint ablation on profile `dryousufmozumder`, mounting `sideview-data` and `reid-checkpoints`. Features `verify_readiness`, `audit_sanity`, Tesla T4 `smoke_test`, and full 30-epoch `main` entrypoints.
- `audit_sideview_viewpoint_transfer.py`: Cross-domain viewpoint transfer sanity audit assessing the frozen real-cattle viewpoint classifier on 50 representative Protocol D train/val crops across the 41 training cows.
- `transfer_viewpoint_to_dryousuf.py`: Direct cloud-to-cloud transfer script safely copying `viewpoint_resnet18_real_best.pth` from `tigerwood693` (`viewpoint-checkpoints`) to `dryousufmozumder` (`reid-checkpoints/viewpoint_aux/`) with bit-identical SHA-256 verification.
- `train_sideview_reid_pose.py`: SideViewCows2026 Re-ID + DeepLabCut SuperAnimal-Quadruped Pose ablation training script featuring 4-channel ResNet-18 visual trunk (`11,179,648` params), Pose MLP branch (`28,736` params), unit-L2 576-D fused embedding, `Linear(576, 41)` classifier (`23,657` params), synchronized pose-aware horizontal flipping, persistent resumable caching, and full Protocol A retrieval evaluation gate. Total trainable parameters: `11,232,041`.
- `modal_train_sideview_reid_pose.py`: Modal cloud wrapper for Re-ID + Pose ablation on profile `tigerwood697`, featuring `verify_readiness`, Tesla T4 `smoke_test`, and full 30-epoch `main` entrypoints on NVIDIA L40S (`timeout=14400`) with persistent pose caching on `/checkpoints/sideview_pose_cache/pose_features_v1.pt`.
...
- `audit_cvb_beef_pose_feasibility.py`: Standalone pose feasibility audit engine evaluating frozen DeepLabCut SuperAnimal-Quadruped ResNet-50 across the authentic primary Behavior stack (CVB + Kaggle Beef, T=8 cattle crops + masks, 54 stratified sequences / 432 frames). Generates frame CSV, sequence summary CSV, aggregate JSON metrics, official 39-keypoint schema, and 9-cell visual contact sheet.
- `modal_audit_cvb_beef_pose.py`: Modal cloud wrapper for CVB + Beef Behavior pose feasibility audit on profile `hasinishrak2015` (Tesla T4), mounting `mtl-data` and `mtl-checkpoints` with automated artifact synchronization back to `artifacts/behavior_pose_audit/`.
- `audit_cvb_beef_viewpoint_transfer.py`: Cross-domain viewpoint transfer sanity audit assessing the frozen real-cattle viewpoint classifier on authentic CVB + Kaggle Beef T=8 sequences (54 stratified sequences / 432 frames; train/val only). Generates frame-level CSV, sequence summary CSV, and comprehensive transfer sanity JSON.
- `train_cvb_beef_behavior_viewpoint.py`: Phase 3 Run 5 Behavior + Viewpoint fused model (`BehaviorTemporalViewpointModel`) and smoke testing script featuring 4-channel ResNet-18 visual trunk (`11,179,648` params), frozen viewpoint ResNet-18 (`11,178,051` params), Viewpoint MLP branch (`400` params, 3 -> 16 -> 16), 528-D fused per-frame feature, and 1D TCN (`740,357` params). Total trainable parameters: `11,920,405` (+16,784 / +0.14% vs Run 5).
- `modal_audit_and_smoke_behavior_viewpoint.py`: Modal cloud wrapper for Behavior viewpoint audit and smoke test on profile `hasinishrak2015` (Tesla T4), mounting `mtl-data` and `mtl-checkpoints` with automated artifact synchronization back to local workspace.
...
- `test_mtl_e4_pcgrad.py`: Exhaustive 12-test unit and architectural verification suite for Phase 3 E4 PCGrad MTL verifying exact parameter counts (11,926,706), absence of adapters/gates, PCGrad conflict projection math, non-conflicting gradient invariance, gradient isolation to shared backbone, unprojected task head gradients (BCS, Behavior, Re-ID), deterministic RNG seed 2026, task forward shapes, bit-identical checkpoint reload (`max_logit_diff == 0.00000000`), and SideView protocol disjointness (41 train vs 69 held-out cows).
- `test_mtl_e3_modular.py`: Focused unit and architectural verification suite for Phase 3 Run 8 E3 Modular MTL (8 tests) verifying parameter counts (12,322,610), adapter zero-weight identity initialization, conv1 4th mask-channel initialization, task forward shapes, strict gradient isolation across task-private adapters, bit-identical checkpoint reload (`max_logit_diff == 0.00000000`), SideView protocol disjointness, and zero unavailable-label padding.
- `test_mtl_e1_evaluation.py`: Unit test suite verifying metric computations, ordinal target expansion, CVB vs Beef dataset breakdowns, and chunked Protocol A retrieval rankings for the MTL E1 held-out evaluation engine.
- `test_reid_pose_ablation.py`: Comprehensive 9-test unit suite for SideView Re-ID + Pose ablation verifying double-flip mathematical involution, anatomical left/right swap mapping, midline isolation, invalid keypoint stability, exact parameter counts (11,232,041), forward shapes, unit-L2 norm, best-to-fresh checkpoint reload determinism, and Protocol A chunked retrieval evaluation functions.
