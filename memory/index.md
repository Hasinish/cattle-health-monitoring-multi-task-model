...
- `build_behavior_perception_cache.py`: Deterministic Run 5 Behavior perception caching engine extracting T=8 temporal frames, applying exact GT bboxes for CVB, and RT-DETR-L largest-box + center point (A5) with center fallback for Kaggle Beef, generating SAM 2.1 binary masks, and persisting exact per-frame `perception_metadata.json` for 100% reproducible resume provenance without placeholder fabrication.
- `train_cvb_beef_behavior_tcn.py`: Run 5 Behavior ResNet-18 + 1D TCN temporal training pipeline supporting both 3-channel RGB and 4-channel RGB+Mask perception modes on canonical CVB + Kaggle Beef sequences.
- `modal_train_cvb_beef_behavior_tcn.py`: Modal cloud execution wrapper for Run 5 temporal core and perception integration on profile `tigerwood693` (NVIDIA T4), supporting remote cache building, model training, and smoke cache audit verification (`audit_smoke_cache`).
