...
- `modal_train_sciencedb_bcs.py`: Modal cloud wrapper for Phase 3 ScienceDB RGB single-task BCS baseline training on Modal (`tigerwood697`, Tesla T4 GPU), mounting `sciencedb-data` at `/data` and `sciencedb-checkpoints` at `/checkpoints`, featuring live batch-level tqdm streaming, automated per-epoch persistent checkpoint commits, and pre-flight readiness audit suite (`verify_readiness`).
