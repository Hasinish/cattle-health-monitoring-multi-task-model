...
- [ ] STEP 2.4 (MOO Synthetic-to-Real Viewpoint Transfer Audit & Scientific Correction, N=100): Ingested official CEA Kalisteo MOO dataset (128k synthetic cattle images, 1,000 IDs) into Modal volume `moo-data` via zero-waste cloud pipeline (`scripts/modal_moo_pipeline.py`, ~$0.30 cost). Trained 5-class linear head on frozen ImageNet ResNet-18 on CPU (93.2% synthetic val acc; checkpoint `artifacts/checkpoints/moo_resnet18_viewpoint.pth`; valid as synthetic-domain validation only). Clean evaluation on N=95 non-ambiguous real cattle crops yielded 12.63% accuracy, Macro-F1 0.0674, and 40 false fronts (clean untuned real transfer result). A subsequent post-hoc 63.16% accuracy / 0.3600 Macro-F1 result was obtained only after testing multiple label permutations against the same real benchmark labels; classified explicitly as an `exploratory post-hoc benchmark fit; invalid as final held-out evaluation evidence`. Independent visual inspection of raw MOO synthetic renders confirmed that MOO `front` is anatomical head, `back` is tail, and `left`/`right` are lateral sides (`Independent evidence does NOT support the claimed 90-degree anatomical mismatch`). Operational viewpoint generator: NOT YET SELECTED. MOO remains an approved roadmap candidate, but current real-transfer evidence is insufficient.
...
- **STEP 2.4 REOPENED | STEP 2 IN PROGRESS**: Viewpoint taxonomy is FEASIBLE; frozen zero-shot REJECTED under tested setup; MOO synthetic validation (93.2%) and raw transfer (12.63%) are clean results, while 63.16% is an exploratory post-hoc benchmark fit (invalid as final test evidence); independent MOO visual check refuted 90° anatomical mismatch; operational viewpoint generator: NOT YET SELECTED.
- Deliverable updated: `docs/audits/phase3_perception_feasibility.md` (Localization, Segmentation, Pose, and Viewpoint Strategy synthesized and committed).
- Immediate next action: Establish a scientifically valid MOO-supervised viewpoint experiment without post-hoc label remapping on the real evaluation benchmark.
