...
## Active Goals & Todo (STEP 1: COMPLETE | GATE 1: CLEARED | STEP 2.1: COMPLETE | STEP 2.2: COMPLETE | STEP 2.3: COMPLETE | STEP 2.4: SYNTHESIZED | STEP 2: IN PROGRESS)
- **Immediate next action:** STEP 2.4 remaining viewpoint feasibility decision: evaluate which remaining roadmap-approved route should be tested next: MOO synthetic supervision OR a supervised cattle-specific viewpoint classifier with adequate/balanced labels.
...
- [x] STEP 2.4 (Manual Taxonomy Review & Operational Strategy Synthesis): Completed manual visual taxonomy review (60 samples verified) and synthesized viewpoint operational strategy in `docs/audits/phase3_perception_feasibility.md`:
  ...
  - MOO-supervised estimator: UNTESTED (canonical roadmap explicitly allows MOO synthetic viewpoint supervision; requires synthetic-to-real transfer experiment).
  - Supervised cattle-specific viewpoint classifier: UNTESTED (canonical roadmap explicitly allows simple classifier if needed; existing 60/100 review labels are severely imbalanced with 0 true front and 2 front-oblique, requiring balanced data and/or MOO synthetic supervision before a proper test).
  - Operational viewpoint generator: NOT YET SELECTED. Step 2.4 and overall Step 2 remain OPEN. Camera ID must NEVER be used as a viewpoint proxy to avoid shortcut leakage.
...
- Completed Step 2.4 (Manual Visual Taxonomy Review, 100-Sample Benchmark, & Operational Strategy Synthesis):
  ...
  - Operational viewpoint generator: NOT YET SELECTED. Step 2.4 and overall Step 2 remain OPEN. Camera ID must NEVER be treated as viewpoint to prevent shortcut leakage.
...
- **STEP 2.4 Viewpoint taxonomy is FEASIBLE; frozen zero-shot REJECTED under tested setup; MOO and supervised classifier UNTESTED; operational generator NOT YET SELECTED; Step 2.4 / Step 2 remain OPEN**.
