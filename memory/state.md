...
- **Immediate next action:** Perform manual human visual review (with Hasin and ChatGPT vision) on the 10 Ruchay 2026 contact sheets (`docs/audits/assets/ruchay_bcs_visual_audit/`) to inspect BCS anatomical feature resolution and flanking stanchion stall occlusion; concurrently resolve operational viewpoint strategy for Step 3 caching.
- [x] STEP 2 Ruchay 2026 RGB Visual Quality Audit Pack: Generated representative 100-sample contact-sheet pack for candidate Primary External BCS validation benchmark Ruchay et al. 2026 (`scripts/build_ruchay_bcs_visual_audit_pack.py`). Stratified across 10 BCS classes (2.75 to 5.00; exactly 10 per class) with seed 2026, maximizing biological cow diversity to 94 unique cows (theoretical maximum). Selectively extracted ~340 MB of 1080p RGB PNGs via buffered HTTP Range streaming over Zenodo archives, eliminating the need to download 77.74 GB of raw data. Generated 10 high-resolution contact sheets (2x5 grid, 640x360 px image tiles, long side 640 px >= 500 px, 1320x2240 px per sheet) with legible metadata banners (Sample ID, BCS, Cow ID, Session, Passage, Filename). Authored full manifest (`artifacts/perception_audit/ruchay_bcs_visual_audit_manifest.csv`) and comprehensive markdown index (`docs/audits/phase3_ruchay_bcs_visual_audit_index.md`). Initial inspection reveals overhead nadir camera framing over parallel milking stalls with prominent dorsal anatomical visibility. Deliverables: `docs/audits/assets/ruchay_bcs_visual_audit/`, `docs/audits/phase3_ruchay_bcs_visual_audit_index.md`, `docs/research_log/2026-09-22_ruchay_bcs_visual_audit_pack.md`.
