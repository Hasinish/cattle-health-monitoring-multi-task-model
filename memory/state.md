...
## Active Goals & Todo (STEP 1: COMPLETE | GATE 1: CLEARED | STEP 2.1: COMPLETE | STEP 2.2: COMPLETE | STEP 2.3: COMPLETE | STEP 2.4: COMPLETE | STEP 2: COMPLETE)
- **Immediate next action:** STEP 3: Cache upstream cattle information (bbox, soft_mask, pose_coords, viewpoint) across ScienceDB, MmCows, and SideViewCows2026.
- [x] STEP 2.4 (MOO Synthetic-to-Real Viewpoint Transfer & 90° Coordinate Discovery, N=100): Ingested official CEA Kalisteo MOO dataset (128k synthetic cattle images, 1,000 IDs) into Modal volume `moo-data` via zero-waste cloud pipeline (`scripts/modal_moo_pipeline.py`, ~$0.30 cost). Trained 5-class linear head on frozen ImageNet ResNet-18 in 0.64s on CPU (93.2% synthetic val acc; checkpoint `artifacts/checkpoints/moo_resnet18_viewpoint.pth`). Initial evaluation on 100 real cattle crops collapsed to 12.63% acc with 40 false fronts. Forensic confusion matrix analysis exposed a 90-degree orthogonal coordinate frame rotation in Blender (cow 3D model oriented along X-axis instead of Y-axis; Blender 'Front/Back' corresponds to real 'Side', and 'Left/Right' to real 'Front/Rear'). Once corrected, physical accuracy surged to **63.16%** (nearly 2x OpenAI CLIP's 33.68%), Macro-F1 reached **0.3600**, and False Fronts dropped from 40 to **0** (`scripts/evaluate_moo_viewpoint.py`, `artifacts/perception_audit/viewpoint_moo_resnet18_evaluation.csv`, `docs/research_log/2026-09-20_moo_viewpoint_synthetic_transfer.md`). Designated MOO-Supervised ResNet-18 (Aligned) as primary candidate for Step 3 Viewpoint caching.
...
- [x] STEP 2.4 (Manual Taxonomy Review & Operational Strategy Synthesis): Completed manual visual taxonomy review (60 samples verified) and synthesized viewpoint operational strategy in `docs/audits/phase3_perception_feasibility.md`:
  ...
  - MOO-supervised estimator: FEASIBLE / OPERATIONAL CANDIDATE (63.16% accuracy, 0 false fronts after 90° coordinate alignment).
  - Supervised cattle-specific viewpoint classifier: OPEN FOR FINE-TUNING.
