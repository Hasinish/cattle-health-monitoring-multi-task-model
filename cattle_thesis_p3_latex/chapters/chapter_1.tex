% Completely rewritten for author review. No commit authorized.
% Project facts checked against main 79998e76780ea21d5f19f08f130768231837ca5a.
% The Phase 2 discussion is supported by P2 chapters 5/6 and the Phase 3 roadmap.
\section{Background}
...
The multi-task stage compares a hard-shared model with a design that permits task-specific processing. These alternatives are assessed against the single-task references using the metric appropriate to each task. Condition scoring is evaluated through the magnitude and ordering of prediction error, behavior through class-sensitive recognition measures, and Re-ID through retrieval performance. Where compatible external data are available, evaluation also examines a change of environment or acquisition setting.
