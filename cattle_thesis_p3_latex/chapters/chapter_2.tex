...
Transfer learning and cattle-centered representation learning are not the same thing. Transfer learning is mainly about where the model's starting weights come from. Cattle-centered learning is about what information the model receives and keeps. The same pretrained encoder can process a full scene, a cropped cow, a masked cow, or an input that also contains anatomical information. Therefore, the encoder can stay the same while the information given to it changes. This distinction matters when we try to understand why one representation works better than another.

\subsection{Prediction Tasks and Their Visual Requirements}

BCS can be treated as classification, ordinal prediction, or regression. Classification treats every score as a separate class. Ordinal prediction also uses the order between scores, while regression predicts a numerical value. This order matters because an error between two neighboring BCS values is different from an error across a large part of the scale. CORAL, for example, uses rank-consistent ordinal outputs for ordered classes~\cite{coral2020}. For BCS, this helps model the order of the labels, but it does not change what visual information is needed from the cow.

Behavior recognition is different because the classes describe activities rather than positions on one numerical scale. A single frame may show a clear posture, while a sequence can also show movement and changes over time. This is important for activities such as standing and walking, which may look similar in one frame. The model therefore needs an observation period that matches the behavior being classified.

Individual identification and Re-ID also need to be separated. A closed-set classifier predicts one of the identities seen during training. In retrieval-based Re-ID, a query image is compared with gallery images using an embedding. This allows evaluation on cows that were not used to train the encoder. A good Re-ID representation should make different images of the same cow similar while keeping different cows separate. Cattle metric-learning research makes this distinction important for herds where animals can change over time~\cite{andrew2021opencows}.

\subsection{Multi-Task Learning and Negative Transfer}

In MTL, several tasks are learned using a model that shares some components. A common training objective combines the task losses,
\begin{equation}
    \mathcal{L}_{\mathrm{MTL}}=\sum_{t=1}^{T}\lambda_t\mathcal{L}_t,
\end{equation}
where $\mathcal{L}_t$ is the loss for task $t$ and $\lambda_t$ controls how much that task contributes. This equation describes the combined loss, but it does not define how much of the network should be shared. A model may share almost everything, only early layers, or selected information between task-specific paths.

Crawshaw's survey~\cite{crawshaw2020mtl} reviews these choices in terms of shared features, task-specific parts, and optimization methods. The benefit of MTL is that one task may help another learn useful features. Negative transfer is the opposite case: a task performs worse in joint training than in a suitable single-task model. This can happen because the shared representation is not suitable, because task gradients interact badly, or because the training objectives are unbalanced. For this thesis, it is important to separate \emph{what information is shared} from \emph{how the shared parameters are optimized}.
