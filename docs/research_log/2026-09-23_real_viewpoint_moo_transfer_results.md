...
1. **Closing the Sim-to-Real Domain Gap:** Pretraining on synthetic 3D cattle CAD geometry (MOO) provides a strong inductive prior for 3D body contours, but synthetic-only representations suffer from severe domain shift when evaluated directly on farm cameras. Fine-tuning on 616 real bounding-box crops adapts the representation to natural textures, illumination, and mud/chute artifacts, raising accuracy from **27.37% to 86.26% (+58.89%)**.
...
4. **Operational Viability for Thesis:** The model satisfies the operational requirements for a real-cattle viewpoint classifier and can serve as an auxiliary representation prior or validation filter.
