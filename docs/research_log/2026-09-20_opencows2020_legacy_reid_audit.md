# OpenCows2020 Legacy Re-ID Evaluation Protocol & Forensic Leakage Audit
...
3. **Severe Flaw in Legacy Preprocessor**: `context/preprocess_id.py` previously executed `random.shuffle()` on image frames within each cow to create train and validation splits. This caused massive temporal and duplicate leakage:
   - **1,023 adjacent-frame pairs** ($|f_1 - f_2| = 1$) crossed between train and validation.
   - **2,942 near-frame pairs** ($|f_1 - f_2| \le 5$) crossed between train and validation.
   - **3 exact-duplicate image pairs** (identical SHA256 hashes) crossed between train and validation.
4. **Rebuilt Protocol**: Rebuilt using **Contiguous Frame-Block Partitioning** (first ~85% of sorted frames to Train, remaining ~15% to Val) combined with **Exact-Duplicate Harmonization**.
   - Adjacent frame boundary transitions reduced from 1,023 down to 48 (single transition per cow).
...
Phase 3 Step 1 of the canonical roadmap requires establishing leakage-safe, reproducible evaluation protocols for all active datasets before any model training begins.
...
The legacy project preprocessor (`context/preprocess_id.py`) randomly shuffled the 4,240 training images within each cow to produce an 85/15 train/val split. However, because animal re-identification imagery extracted from video tracking often contains adjacent or near-duplicate frames, random splitting introduces severe data leakage, artificially inflating validation accuracy. This audit investigated the source structure to determine whether true sequence/tracklet grouping could be reconstructed, quantified the leakage, and established a defensible evaluation protocol.
...
### 3.4 Flaw in Legacy Split (`preprocess_id.py`)
Because `context/preprocess_id.py` shuffled frames randomly:
- **1,023 adjacent-frame pairs** ($|f_1 - f_2| = 1$) crossed between train and validation.
- **2,942 near-frame pairs** ($|f_1 - f_2| \le 5$) crossed between train and validation.
- **1,760 near-duplicate image pairs** (MAE < 15) crossed between train and validation (2.07% of all train-val pairs).
- **3 exact duplicate pairs** (Cow 13 frames 26/172, Cow 2 frames 71/94, Cow 7 frames 48/123) crossed between train and validation.
...
2. **Contiguous Frame-Block Partitioning**:
   ...
   - Adjacent frame boundary transitions are reduced from 1,023 down to 48 (single boundary transition per cow).
...
5. **Defensible Heuristic, Not True Sequence Split**:
   - Because true tracklets cannot be proven without inventing fake provenance, we explicitly acknowledge that contiguous block splitting is a defensible heuristic to prevent frame-hopping, but does NOT guarantee camera- or flight-disjoint evaluation.
...
| `datasets/id/opencow2020/train.csv` | Leakage-free training split | 3,586 rows |
| `datasets/id/opencow2020/val.csv` | Leakage-free validation split | 654 rows |
...
| `scripts/verify_opencows_splits.py` | Standalone verification script asserting counts, IDs, and 0 duplicate leakage | Python |
