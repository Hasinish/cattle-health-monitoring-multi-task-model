...
DRYAD_CHECKPOINT = os.path.join(WORKSPACE_DIR, "dryad_bcs_ce_best.pth")
RESULTS_TXT = os.path.join(WORKSPACE_DIR, "bcs_ce_ablation_results.txt")
LOSS_CURVE_PNG = os.path.join(WORKSPACE_DIR, "bcs_ce_ablation_loss_curve.png")
...
def run_training(dataset_name, csv_path, label_map, checkpoint_path):
    print("=" * 80)
    print(f"STARTING ABLATION (CE): {dataset_name}")
...
def write_ablation_report(dryad_result):
    report = f"""---CONTEXT 3 BCS CE ABLATION---
PERSON NAME: {PERSON_NAME}
BASE MODEL: {BASE_MODEL}
DATASET: {dryad_result['dataset_name']} (Cross-Entropy Loss)
...
"""
    ...
    print(f"CE Ablation report saved to: {RESULTS_TXT}")
    print(report)
...
def save_loss_curve(dryad_result):
    ...
    plt.plot(
        ...
        label="Dryad CE Train Loss",
    )
    plt.plot(
        epochs,
        dryad_result["val_losses"],
        label="Dryad CE Val Loss",
    )

    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("BCS CE Ablation Training and Validation Loss Curves")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
