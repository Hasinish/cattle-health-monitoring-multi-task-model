...
CHECKPOINT_PATH = r"D:\T25301094 P2\workspaces\hasin\id_best.pth"
RESULTS_PATH = r"D:\T25301094 P2\workspaces\hasin\id_results.txt"
LOSS_CURVE_PATH = r"D:\T25301094 P2\workspaces\hasin\id_loss_curve.png"
...
def save_loss_curve(train_losses):
    ...
    plt.title(f"Cow ID Training Loss Curve — {PERSON_NAME} ({BASE_MODEL_DISPLAY})")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(LOSS_CURVE_PATH, dpi=300)
    plt.close()

def write_results(actual_epochs_trained, epoch_losses, final_train_loss, val_acc, test_acc, training_time_mins):
    def fmt(v):
...
LOSS AT EPOCH 10: {fmt(epoch_losses.get(10))}
...
def main():
    ...
    model = CowIDModel(MODEL_NAME, NUM_CLASSES, device).to(device, non_blocking=True)

    # Freeze backbone — train only CBAM + classifier head
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        scheduler.step()
        print(f"Epoch {epoch}/{MAX_EPOCHS} | Loss: {train_loss:.6f} | Val Top-1: {val_acc*100:.2f}%")
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), CHECKPOINT_PATH)

    model.load_state_dict(torch.load(CHECKPOINT_PATH, weights_only=True))
    val_acc = evaluate(model, val_loader, device, "Best Val Eval")
    test_acc = evaluate(model, test_loader, device, "Test Eval")
    save_loss_curve(train_losses)
    training_time_mins = (time.time() - start_time) / 60
    write_results(MAX_EPOCHS, epoch_losses, train_losses[-1], val_acc, test_acc, training_time_mins)
